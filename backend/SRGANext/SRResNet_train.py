...
for epoch in range(num_epochs):
    for i, (lr_images, hr_images) in enumerate(train_loader):
        ...
        content_loss_vgg_value = 1e-3 * content_loss_vgg(high_res_vgg, generated_high_res_vgg)
    ...
    with torch.no_grad():
        ...
        save_image(hr_images, 'output/SRResNet/hr_images_{}.png'.format(epoch + 1))
        save_image(lr_images, 'output/SRResNet/lr_images_{}.png'.format(epoch + 1))
        save_image(fake_hr_images, 'output/SRResNet/fake_hr_images_{}.png'.format(epoch + 1))
        if (epoch+1) % 10 == 0:
            torch.save(generator.state_dict(), 'modules/SRResNet_weight/generator_{}.pth'.format(epoch+1))