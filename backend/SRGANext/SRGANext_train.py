...
from utils.SRGANext_module import Generator, Discriminator
...
for epoch in range(num_epochs):
    for i, (lr_images, hr_images) in enumerate(train_loader):
        ...
        # 计算VGG19损失
        # # Get the feature map of the HR image
        hr_feature_map = vgg19(hr_images)
        # Get the feature map of the generated image
        gen_feature_map = vgg19(generated_high_res)
        # Extract the feature map before active layer
        hr_feature_map_before_active = hr_feature_map[:, :, :, :-1]
        gen_feature_map_before_active = gen_feature_map[:, :, :, :-1]
        content_loss_vgg_value = 1e-4 * content_loss_vgg(hr_feature_map_before_active, gen_feature_map_before_active)
        ...
        real_loss = adversarial_loss(dis_hr,
                                     (torch.ones(batch_size, 1) - 1e-5 * torch.rand(batch_size, 1)).to(device))
        fake_loss = adversarial_loss(dis_generated,
                                     (torch.zeros(batch_size, 1) + 1e-5 * torch.rand(batch_size, 1)).to(device))
    ...
    with torch.no_grad():
        ...
        save_image(hr_images, 'output/SRGANext/hr_images_{}.png'.format(epoch + 1))
        save_image(lr_images, 'output/SRGANext/lr_images_{}.png'.format(epoch + 1))
        save_image(fake_hr_images, 'output/SRGANext/fake_hr_images_{}.png'.format(epoch + 1))
        if (epoch+1) % 10 == 0:
            torch.save(generator.state_dict(), 'modules/SRGANext_weight/generator_{}.pth'.format(epoch + 1))
            torch.save(discriminator.state_dict(), 'modules/SRGANext_weight/discriminator_{}.pth'.format(epoch + 1))